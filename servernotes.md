...
$GLOBALS['DB']->query("SELECT courses.* FROM accounts JOIN accounts_courses ON (accounts.id = accounts_courses.account_id) JOIN courses ON (accounts_courses.course_id = accounts.id) WHERE courses.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT images.* FROM accounts JOIN accounts_images ON (accounts.id = accounts_images.account_id) JOIN images ON (accounts_images.image_id = accounts.id) WHERE images.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT lessons.* FROM accounts JOIN accounts_lessons ON (accounts.id = accounts_lessons.account_id) JOIN lessons ON (accounts_lessons.lesson_id = accounts.id) WHERE lessons.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT schools.* FROM accounts JOIN accounts_schools ON (accounts.id = accounts_schools.account_id) JOIN schools ON (accounts_schools.school_id = accounts.id) WHERE schools.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT services.* FROM accounts JOIN accounts_services ON (accounts.id = accounts_services.account_id) JOIN services ON (accounts_services.service_id = accounts.id) WHERE services.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT students.* FROM accounts JOIN accounts_students ON (accounts.id = accounts_students.account_id) JOIN students ON (accounts_students.student_id = accounts.id) WHERE students.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT teachers.* FROM accounts JOIN accounts_teachers ON (accounts.id = accounts_teachers.account_id) JOIN teachers ON (accounts_teachers.teacher_id = accounts.id) WHERE teachers.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT images.* FROM courses JOIN courses_images ON (courses.id = courses_images.course_id) JOIN images ON (courses_images.image_id = courses.id) WHERE images.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT lessons.* FROM courses JOIN courses_lessons ON (courses.id = courses_lessons.course_id) JOIN lessons ON (courses_lessons.lesson_id = courses.id) WHERE lessons.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT schools.* FROM courses JOIN courses_schools ON (courses.id = courses_schools.course_id) JOIN schools ON (courses_schools.school_id = courses.id) WHERE schools.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT services.* FROM courses JOIN courses_services ON (courses.id = courses_services.course_id) JOIN services ON (courses_services.service_id = courses.id) WHERE services.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT students.* FROM courses JOIN courses_students ON (courses.id = courses_students.course_id) JOIN students ON (courses_students.student_id = courses.id) WHERE students.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT teachers.* FROM courses JOIN courses_teachers ON (courses.id = courses_teachers.course_id) JOIN teachers ON (courses_teachers.teacher_id = courses.id) WHERE teachers.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT lessons.* FROM images JOIN images_lessons ON (images.id = images_lessons.image_id) JOIN lessons ON (images_lessons.lesson_id = images.id) WHERE lessons.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT schools.* FROM images JOIN images_schools ON (images.id = images_schools.image_id) JOIN schools ON (images_schools.school_id = images.id) WHERE schools.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT services.* FROM images JOIN images_services ON (images.id = images_services.image_id) JOIN services ON (images_services.service_id = images.id) WHERE services.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT students.* FROM images JOIN images_students ON (images.id = images_students.image_id) JOIN students ON (images_students.student_id = images.id) WHERE students.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT teachers.* FROM images JOIN images_teachers ON (images.id = images_teachers.image_id) JOIN teachers ON (images_teachers.teacher_id = images.id) WHERE teachers.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT schools.* FROM lessons JOIN lessons_schools ON (lessons.id = lessons_schools.lesson_id) JOIN schools ON (lessons_schools.school_id = lessons.id) WHERE schools.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT services.* FROM lessons JOIN lessons_services ON (lessons.id = lessons_services.lesson_id) JOIN services ON (lessons_services.service_id = lessons.id) WHERE services.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT students.* FROM lessons JOIN lessons_students ON (lessons.id = lessons_students.lesson_id) JOIN students ON (lessons_students.student_id = lessons.id) WHERE students.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT teachers.* FROM lessons JOIN lessons_teachers ON (lessons.id = lessons_teachers.lesson_id) JOIN teachers ON (lessons_teachers.teacher_id = lessons.id) WHERE teachers.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT services.* FROM schools JOIN schools_services ON (schools.id = schools_services.school_id) JOIN services ON (schools_services.service_id = schools.id) WHERE services.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT students.* FROM schools JOIN schools_students ON (schools.id = schools_students.school_id) JOIN students ON (schools_students.student_id = schools.id) WHERE students.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT teachers.* FROM schools JOIN schools_teachers ON (schools.id = schools_teachers.school_id) JOIN teachers ON (schools_teachers.teacher_id = schools.id) WHERE teachers.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT students.* FROM services JOIN services_students ON (services.id = services_students.service_id) JOIN students ON (services_students.student_id = services.id) WHERE students.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT teachers.* FROM services JOIN services_teachers ON (services.id = services_teachers.service_id) JOIN teachers ON (services_teachers.teacher_id = services.id) WHERE teachers.id = {$this->getId()};");
$GLOBALS['DB']->query("SELECT teachers.* FROM students JOIN students_teachers ON (students.id = students_teachers.student_id) JOIN teachers ON (students_teachers.teacher_id = students.id) WHERE teachers.id = {$this->getId()};");
